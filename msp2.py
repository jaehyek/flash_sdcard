...
#  $Header: //components/rel/config.glue/1.0/storage/msp2.py#2 $
#  $DateTime: 2015/01/30 18:02:55 $ 
#  $Author: pwbldsvc $
...
def PrintBigError(sz):
    ...
    if len(sz)>0:
        device_log(sz)
        device_log("\nmsp2.py failed - Log is log_msp.txt\n\n")
        sys.exit(1)
    ...
    for ReadCmd in ReadArray:
        ...
        if interactive is True:
            ...
            if loadfile=='Y' or loadfile=='y' or loadfile=='':
                ...
                device_log("\nmsp2.py exiting by user pressing Q (quit) - Log is log_msp.txt\n\n")
                sys.exit()
            else:
                continue
        ...
        try:
            ...
            if sys.platform.startswith("linux"):
                ...
                device_log("\tsudo python msp2.py partition.xml /dev/sdx (where x is the device node)")
            else:
        ...
        if readsector_fromsector == True :
            """
            read more based on the bytes_read and save to ReadCmd['filename']+"log" .
            """

            bytes_read = bytes_read[0:8]

            sectorno_blockdata, sectorlen_blockdata = struct.unpack("<LL",bytes_read )
            ipfile = open(ReadCmd['filename']+".log", "wb")
            opfile = open(Filename, "r+b")
            opfile.seek(int(sectorno_blockdata * SECTOR_SIZE))
            size = SECTOR_SIZE
            for i in range (sectorlen_blockdata ) :
                bytes_read = opfile.read(size)
                ipfile.write(bytes_read)
            ipfile.close()
            opfile.close()
    ...
    for Write in WriteSorted:   # Here Write is a *sorted* entry from rawprogram.xml
        ...
        while FileWithPath is None:
            ...
            if temppath=='Q' or temppath=='q' or temppath=='':
                device_log("\nmsp2.py exiting - user pressed Q (quit) - Log is log_msp.txt\n\n")
                sys.exit()
        ...
        if noprompt is True:
            ...
            if var=='N' or var=='n':
                ...
                if var=='Y' or var=='y':
                    ...
                    device_log("\nmsp2.py exiting - user didn't want to patch - Log is log_msp.txt\n\n")
                    sys.exit(1)
...
def Usage():
    ...
    print "%-40s\t python msp2.py" % "Display this info"
    print "%-40s\t%s python msp2.py -r wipe_rawprogram_PHY0.xml -d %s" % ("Wipe partition info (-w)",sudo,drive)
    print "\n%-40s\t%s python msp2.py -r rawprogram0.xml -d %s" % ("Write a device (-r)",sudo,drive)
    print "%-40s\t%s python msp2.py -r rawprogram0.xml -d 0" % ("Create a singleimage.bin (-r)",sudo)
    print "%-40s\t%s python msp2.py -r rawprogram0.xml -d 0 -t c:\\temp" % ("singleimage.bin stored to c:\\temp (-r)",sudo)
    print "%-40s\t%s python msp2.py -r rawprogram0.xml -d 16777216" % ("Create an 8GB singleimage.bin (-r)",sudo)
    print "\n%-40s\t%s python msp2.py -n -r rawprogram.xml -d %s" % ("no prompts (-n) i.e. automation",sudo,drive)
    print "%-40s\t%s python msp2.py -i -r rawprogram.xml -d %s " % ("Interactively choose files (-i)",sudo,drive)
    print "%-40s\t%s python msp2.py -f sbl1.mbn,sbl2.mbn -r rawprogram.xml -d %s " % ("Specify files from rawprogram.xml (-f)",sudo,drive)
    print "%-40s\t%s python msp2.py -s c:\windows,d:\ -r rawprogram.xml -d %s " % ("Search this path for files (-s)",sudo,drive)
    print "\n%-40s\t%s python msp2.py -p patch0.xml -d %s" % ("Patch a device (-p)",sudo,drive)
    print "%-40s\t%s python msp2.py -p patch0.xml -d singleimage.bin" % ("Patch files to singleimage (-p)",sudo)
    print "%-40s\t%s python msp2.py -p patch0.xml -d 16777216" % ("PRE-PATCH images for an 8GB disk (-p)",sudo)
    print "\n%-40s\t%s python msp2.py -r rawprogram0.xml -p patch0.xml -d %s" % ("ALL IN ONE STEP",sudo,drive)

    print "============== adding a reading function ===========================\n"
    print "%-40s\t%s python msp2.py -g read0.xml -d %s" % ("reading and saving from device",sudo, drive)
...
try:
    opts, args = getopt.getopt(sys.argv[1:], "r:p:d:inxs:f:t:vb:g:", ["rawprogram=", "patch=", "dest=","noprompt=", "interactive=", "search_path=","file=","location=","verbose","sectorsize","get="])
...
for o, a in opts:
    if o in ("-r", "--rawprogram"):
        ...
        get_filename = a
        Operation |= OPERATION_READ  ## assumed here. It will be corrected later
    elif o in ("-x"):
        readsector_fromsector = True
    else:
...
if Operation==0:    ## Means nothing was specified above
    GetPartitions()
    device_log("\nmsp2.py exiting - Only showed options and drives detected - Log is log_msp.txt\n\n")
    if verbose is True:
        device_log("\nMass Storage Programmer (msp2.py) VERSION 1.0\n");
    sys.exit(1)
...
for Write in WriteArray:
    ...
    if 'sparse' in Write:
        ...
        if type(m) is not NoneType:
            ...
            device_log("opened and parsed. Then you can run msp2.py again to program.")
        ...
        if noprompt is False:
            if (DiskSizeInBytes/(1024.0*1024.0))>100:
                ...
                if var=='Y' or var=='y' or var=='':
                    ...
                    device_log("\nmsp2.py exiting - User didn't want to continue - Log is log_msp.txt\n\n")
                    sys.exit()
    else:
        if noprompt is True:
            # means don't bug them, i.e. automation
...
if (Operation & OPERATION_PROGRAM) > 0 and (Operation & OPERATION_PATCH) > 0:
    ...
    device_log("\tEx: %spython msp2.py -p patch0.xml -d %s " % (sudo,Filename))
    device_log("\t\tOr, do it all in one step")
    device_log("\tEx: %spython msp2.py -r rawprogram0.xml -d %s -p patch0.xml" % (sudo,Filename))
    device_log("-"*78)
